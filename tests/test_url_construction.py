...
async def test_get_wayback_noframe_server_url_with_slash(client):
    ...
    """
    with patch.dict(
        "memorial.app.config", {"WAYBACK_NOFRAME_SERVER": "https://arquivo.pt/noFrame/replay"}, clear=False
    ):
...
async def test_get_host_configuration_strips_www(client):
    ...
    with patch.dict(
        "memorial.app.config",
        {"ARCHIVE_CONFIG": {"example.com": {"message_pt": "Test"}}, "STRIP_PORT": False},
        clear=False,
    ):
        # Test with www
...
async def test_get_host_configuration_no_config(client):
    ...
    with patch.dict("memorial.app.config", {"ARCHIVE_CONFIG": {}, "STRIP_PORT": False}, clear=False):
...
async def test_get_host_configuration_with_port_stripping(client):
    ...
    with patch.dict(
        "memorial.app.config",
        {"ARCHIVE_CONFIG": {"example.com": {"message_pt": "Test"}}, "STRIP_PORT": True},
        clear=False,
    ):
        # Test with port
