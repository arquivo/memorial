...
async def test_default_messages_status_200_all_parts(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "test-200.example.com", expected_status=200)
        ...
        assert (
            "desactivado" in messages_pt["primary_message"].lower()
        ...
        assert (
            "memorial" in messages_pt["message_before_button"].lower()
        ), f"PT message_before_button should contain 'memorial', got: {messages_pt['message_before_button']}"
        assert (
            "preservou" in messages_pt["message_before_button"].lower()
        ...
        assert (
            "arquivo" in messages_pt["button_message"].lower()
        ...
        assert (
            "disabled" in messages_en["primary_message"].lower()
        ...
        assert (
            "preserved" in messages_en["message_before_button"].lower()
        ...
        assert (
            "browse" in messages_en["button_message"].lower()
...
async def test_default_messages_status_502_all_parts(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "test-502.example.com", expected_status=502)
        ...
        assert messages_pt["primary_message"] is not None, "PT primary message should exist"
        assert (
            "temporariamente" in messages_pt["primary_message"].lower()
            or "indisponível" in messages_pt["primary_message"].lower()
        ...
        assert messages_en["primary_message"] is not None, "EN primary message should exist"
        assert (
            "temporarily" in messages_en["primary_message"].lower()
            or "unavailable" in messages_en["primary_message"].lower()
...
async def test_custom_primary_message_with_default_button_messages(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "custom-primary.example.com", expected_status=200)
...
async def test_button_message_contains_valid_text(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "button-test.example.com", expected_status=200)
        ...
        assert len(messages_pt["button_message"].strip()) > 0, "PT button_message should not be empty"

        # Check English button
        messages_en = extract_message_parts(data, "en")
        assert messages_en["button_message"] is not None, "EN button_message should not be None"
        assert len(messages_en["button_message"].strip()) > 0, "EN button_message should not be empty"
...
async def test_message_before_button_contains_link(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "link-test.example.com", expected_status=200)
        ...
        assert (
            "arquivo.pt/memorial" in link.attrs["href"]
...
async def test_message_before_button_with_partial_custom_message(client):
    ...
    with with_test_config(
        ...
        response = await request_host(client, "/", "partial-custom.example.com", expected_status=502)

        data = await response.data

        # Check Portuguese messages
        messages_pt = extract_message_parts(data, "pt")
        assert messages_pt["primary_message"] == custom_pt, "PT primary message should be custom"
        assert messages_pt["message_before_button"] is not None, "PT message_before_button should use default"
        assert (
            "memorial" in messages_pt["message_before_button"].lower()
        ...
        assert messages_en["message_before_button"] is not None, "EN message_before_button should use default"
        assert messages_en["button_message"] is not None, "EN button_message should use default"
